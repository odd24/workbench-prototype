...
function inlineMarkdownToHtml(text) {
  return markdownCore.inlineMarkdownToHtml(text, {renderReference:referenceTokenToHtml, renderImage:markdownImageHtml});
}

function parseReferenceToken(token = '') {
  return markdownCore.parseReferenceToken(token);
}
...
function referenceTokenDetails(token) {
  return markdownCore.referenceTokenDetails(token, referenceTarget);
}

function referenceTokenToHtml(token) {
  return markdownCore.referenceTokenToHtml(token, referenceTarget);
}
...
function unescapeHtml(value) {
  return markdownCore.unescapeHtml(value);
}

function normalizeCodeLanguage(language = 'txt') {
  return markdownCore.normalizeCodeLanguage(language);
}

function syntaxHighlightCode(code, language = 'txt') {
  return markdownCore.syntaxHighlightCode(code, language);
}

function splitMarkdownTableRow(line) {
  return markdownCore.splitMarkdownTableRow(line);
}

function markdownTableAlignment(cell) {
  return markdownCore.markdownTableAlignment(cell);
}

function markdownTableHtml(lines, startIndex) {
  return markdownCore.markdownTableHtml(lines, startIndex, {renderReference:referenceTokenToHtml, renderImage:markdownImageHtml});
}

function markdownToHtml(markdown = '', interactive = false) {
  return markdownCore.markdownToHtml(markdown, {interactive, renderReference:referenceTokenToHtml, renderImage:markdownImageHtml});
}

function cssColorToHex(value = '') {
  return markdownCore.cssColorToHex(value);
}

function codeElementToText(root) {
  return documentModelCore.codeElementToText(root);
}

function hydrateEditorBlocks(editor) {
  return documentModelCore.hydrateEditorBlocks(editor);
}
...
function markdownToPlainText(markdown = '', maxLength = 110, title = '') {
  return markdownCore.markdownToPlainText(markdown, maxLength, title, referenceTarget);
}
