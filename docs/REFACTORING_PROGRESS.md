...
| RF-505 | 文档和发布收尾 | 已完成 | 2026-09-17 | 2026-09-17 | Codex | 当前架构、数据格式、维护说明、便携备份与最终发布门禁全部对齐 |
...
| 2026-09-17 | RF-505 | 待开始 → 进行中 | 计划以代码事实审计并更新 README 启动/功能/数据/验证说明、AGENTS 当前模块边界与测试基线，新建或补齐架构、数据格式和维护发布说明；核对重构目标结构、兼容门面、脚本加载顺序、资源版本、忽略规则、死代码和生成物；仅删除经引用搜索与门禁确认无用的兼容层/死代码 | 计划运行文档链接和路径审计、兼容导出/全局名称引用搜索、90 项 Python、全部 Node 契约、真实浏览器工作流、性能小样本、视觉基线结构验收、全文件语法检查和 `git diff --check` | 风险：兼容导出与全局名称可能被测试或本地调用依赖，无法证明无用则保留并记录；本项不改变数据格式、API、UI 或启动方式，不为“整洁”删除旧数据读取逻辑 |
| 2026-09-17 | RF-505 | 进行中 → 已完成 | 重写 README 并新增架构、数据格式和维护说明；按当前代码更新 AGENTS 与重构计划基线说明；审计全部 `Workbench` 模块、`server.py` 兼容导出、脚本/CSS 顺序和本地生成物；完整备份排除 `config/settings.json` 与 `.workbench-*` 机器专属文件，新增回归测试；同步应用版本与性能基线 | 91 项 Python、全部 13 个 Node 契约和真实浏览器工作流通过；固定样本四套环境共 32 张视觉/DOM 基线通过；默认规模性能验收的磁盘重读、1620 个 ZIP 条目、HTTP 与无头浏览器门禁通过；27 个 Python、37 个 JavaScript 语法检查、17 个 Markdown 本地链接和 `git diff --check` 通过 | 磁盘数据格式、现有 API、UI、启动方式与旧格式读取不变；便携 ZIP 不再泄露本机绝对路径；引用仍在使用的兼容门面和普通脚本全局全部保留；DEC-029、DEC-030；本轮 RF-000 至 RF-505 全部完成 |
...
| 2026-09-17 | RF-505 | `python -m unittest -v`、全部 13 个 `test*.js` | 通过，91 项 Python | 新增便携备份机器配置排除测试；仓储、HTTP、安全边界及真实浏览器保存/重开/草稿/历史/增量刷新/冲突矩阵无回归 |
| 2026-09-17 | RF-505 | `node visual_baseline.js`（固定样本） | 通过，32 张临时截图 | 浅色/深色 × 桌面/窄屏的首页、项目、记录、知识库、文档、概念图、搜索和交互态 DOM 断言通过；产物已从系统临时目录清理 |
| 2026-09-17 | RF-505 | `python performance_acceptance.py` | 通过 | 默认大样本磁盘重读、正文哈希、120 附件、1620 条便携 ZIP/CRC、1200 条签名、500/499 图数据及浏览器 30 秒门禁全部通过；实测值已同步性能文档 |
| 2026-09-17 | RF-505 | 全文件语法、Markdown 本地链接与 `git diff --check` | 通过 | 27 个 Python、37 个 JavaScript、17 个 Markdown 文件；所有本地链接存在，仅有 Git 行尾转换提示 |
...
| DEC-029 | 2026-09-17 | 本轮保留 `server.py` 兼容导出/装配子类与 `window.Workbench` 普通脚本接口 | 引用审计确认测试、动态 patch 点和模块装配仍依赖这些入口；无法证明无用时删除会制造不必要的兼容风险 | 当前架构文档明确实现位置与兼容边界；未来移除必须作为独立工作项迁移调用方并更新契约 |
| DEC-030 | 2026-09-17 | 完整便携备份排除 `config/settings.json` 和所有 `.workbench-*` 机器专属文件 | 这些文件可能包含本机数据目录、导出目录或编辑器绝对路径，不属于可移植业务数据 | 已有磁盘配置继续原位可读写；项目/知识库内容和便携配置照常导出，新增测试锁定归档边界 |
...
| RISK-003 | `app.js` 大量全局状态和集中事件 | 移动代码可能产生重复绑定或异步覆盖 | RF-302 状态/请求边界、RF-303 稳定事件控制器与 RF-307 生命周期装配已落地；剩余 DOM 适配继续按独立边界移动 | 已缓解 |
| RISK-004 | CSS 存在后置覆盖 | 拆分顺序变化可能产生视觉回归 | RF-401 基线、RF-403 十文件拆分与 RF-404 单一响应式入口已落地，样式契约持续锁定加载/断点顺序 | 已缓解 |
...
| ID | 事项 | 触发条件 | 处理状态 |
| --- | --- | --- | --- |
| EVAL-001 | 是否在模块稳定后切换原生 ES Modules | RF-307 完成后 | 已评估：本轮保留普通脚本与 `window.Workbench`，避免引入部署/加载迁移；未来作为独立工作项评估 |
| EVAL-002 | 是否引入浏览器自动化测试依赖 | RF-205 手工矩阵维护成本过高时 | 已评估：采用 Node 标准库 + 系统浏览器 DevTools，不引入第三方依赖 |
| EVAL-003 | 是否把测试移动到 `tests/` 包 | 新增多个测试文件后发现根目录难以维护 | 已评估：本轮保留根目录入口以兼容现有命令与脚本；未来迁移需单独修改发现路径和文档 |
